...
INSERT INTO reviews(series_id, reviewer_id, rating) VALUES
   ...
   select  title, avg(rating) from series
     join  reviews  on 
   series.id=reviews.series_id  group by title order by avg(rating);
   
   select first_name,last_name,rating from reviewers  right join reviews on
   ...
   select title,rating, concat(first_name,last_name) as viewier from series join reviews join reviewers on series.id=reviews.series_id=reviewers.id;